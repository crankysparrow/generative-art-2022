import { slider, sliderOpts } from './controls/slider'

export class Controls {
	shown: boolean
	container: {}
	count: number
	activePanel: boolean | number
	panels = {}
	panelIds = []
	tabBtns = []
	...
	buildContainer() {
		...
		// .position(this.rightSide ? window.innerWidth - 300 : 0, 0, 'right')
	...
	addPanel({ title, id, defaultOpen = true }) {
		this.count++
		...
		let tabBtn = createButton(title)
			...
			.attribute('aria-expanded', defaultOpen)
		...
		let panel = createDiv()
			.addClass('panel')
			.attribute('id', id)
			.attribute('aria-hidden', !defaultOpen)
			.parent(this.container)
	...
	createSlider(obj = {}, prop: string | number, opts: sliderOpts) {}
