...
function curves(w, h) {
	...
	let t_max = round(random(7, 15), 1)
	let t_min = round(random(1.5, 5), 1)
	let steps = 20

	let c = 0
	while (c <= 1) {
		let t = map(easeInCubic(c), 0, 1, t_min, t_max)
		let x = -2
		let vals = []

		beginShape()
		while (x <= 2) {
			let y = pow(1 - pow(abs(x / 2), t), 1 / t)
			let px = x * w * map((t - 1) / 6, 0, 1, scaleX_min, scaleX_max)
			// let px = map(x, -2, 2, -w / 2, w / 2)
			// --> this is the same as x * w * scaleX if scaleX = 0.25

			let py = y * h * map(1 - (t - 1) / 6, 0, 1, scaleY_min, scaleY_max)
			vals.unshift([px, -py])
			vertex(px, py)
			x += 0.01
